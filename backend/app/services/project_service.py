...
async def get_stats(db: AsyncSession, user_id: str | None = None) -> ProjectStats:
    ...
    scoped = base_query.subquery()
    words_result = await db.execute(select(func.sum(scoped.c.word_count)))
    total_words = words_result.scalar_one() or 0

    platforms_result = await db.execute(
        select(scoped.c.platform).distinct()
    )
    platforms_used = sorted(set(row[0] for row in platforms_result.all() if row[0]))

    last_result = await db.execute(
        select(scoped.c.created_at).order_by(scoped.c.created_at.desc()).limit(1)
    )
...
async def duplicate_project(
    db: AsyncSession, project: Project, user_id: str | None = None
) -> Project:
